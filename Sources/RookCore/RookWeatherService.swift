...
enum RookWeatherError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .locationPermissionRequired:
            return "Location access is off. Enable it in System Settings, or say a city such as ‘weather in Oakland, New Jersey.’"
        case .locationUnavailable:
...
final class RookWeatherService: NSObject, @preconcurrency CLLocationManagerDelegate {
    ...
    private func waitForDeviceLocation(
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + locationLimit) { [weak self] in
            guard let self,
                  let index = self.deviceWaiters.firstIndex(where: { $0.id == id }) else { return }
            ...
            case .failure:
                self.deviceWaiters.removeAll { waiter in
                    ...
                    if let fallback, Date().timeIntervalSince(fallback.fetchedAt) <= self.staleFallbackInterval {
                        waiter.completion(.success(self.response(from: fallback, request: waiter.request, startedAt: waiter.startedAt)))
                    } else {
    ...
    private func fetchForecastData(
        ...
        components.queryItems = [
            ...
            URLQueryItem(name: "current", value: "temperature_2m,apparent_temperature,is_day,precipitation,rain,weather_code,wind_speed_10m"),
    ...
    private func fetchJSON<Value: Decodable>(
        ...
        request.setValue("Rook/2.12 personal weather assistant", forHTTPHeaderField: "User-Agent")
    ...
    private func response(
        ...
        if isCurrent, let index = indices.first {
            ...
            displayText = "**\(entry.label):** \(current)°F and \(condition). Feels like \(feels)°. Today’s high is \(high)°, low \(low)°, with a \(rainChance)% chance of rain."
            spokenText = "It’s \(current) degrees and \(condition) \(spokenLocation). Today’s high is \(high), with a \(rainChance) percent chance of rain."
            ...
            displayText = "**\(entry.label):** your \(request.dayCount == 1 ? "forecast" : "\(request.dayCount)-day forecast") is ready."
            spokenText = "The forecast for \(entry.label) is ready. The first day is \(condition), with a high of \(high) and a \(rainChance) percent chance of rain."
        } else {
        ...
        let freshness = Date().timeIntervalSince(entry.fetchedAt) <= freshInterval ? "Live forecast" : "Recent cached forecast"
        return RookResponse(
    ...
    private func loadCache() {
        guard let data = try? Data(contentsOf: config.weatherCacheURL),
              let document = try? JSONDecoder().decode(RookWeatherCacheDocument.self, from: data) else { return }
