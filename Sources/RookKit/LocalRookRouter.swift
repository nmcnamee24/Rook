...
public enum LocalRookRouter {
    ...
    private static func instantAnswer(for command: String) -> (display: String, spoken: String)? {
        ...
        if containsAny(command, ["what can you do", "what do you do", "how can you help"]) {
            let display = "I can answer questions immediately, then quietly deploy independent pawn crews for research, code, writing, Calendar, Gmail, and verification when the request needs real work."
    ...
    private static func needsDeliberation(_ command: String) -> Bool {
        if containsAny(command, [
            ...
            "research", "look up", "find out", "compare", "recommend", "latest", "current", "news", "weather", "forecast", "temperature",
            ...
            "bug", "error", "link opening", "interrupted", "blocked", "pawn", "crew", "forge", "scribe", "steward", "auditor", "scout",
    ...
    private static func pawnPlan(for command: String) -> [PawnPlan] {
        ...
        for clause in clauses {
            ...
            if containsAny(normalized, ["calendar", "schedule", "reschedule", "work block", "appointment", "meeting", "availability", "gmail", "inbox", "email", "reply"]) {
                roles.append("Steward")
            }
            if containsAny(normalized, ["document", "notes", "outline", "write", "draft", "rewrite", "summary", "summarize"]) {
                roles.append("Scribe")
            }
            if containsAny(normalized, ["code", "repository", "repo", "project", "file", "debug", "fix", "implement", "build", "deploy", "app", "bug", "error", "link opening", "forge"]) {
                roles.append("Forge")
            }
            if containsAny(normalized, ["research", "look up", "find", "compare", "recommend", "latest", "current", "news", "weather", "forecast", "temperature", "picture", "photo", "image", "diagram", "visualize", "flowchart", "location", "options", "context", "librarian", "library", "remember", "memory", "history", "last time", "previous", "previous task"]) {
                roles.append("Scout")
            }
            if containsAny(normalized, ["verify", "audit", "double check", "make sure", "conflict", "risk", "review", "approval", "approve", "delete", "purchase", "book", "apply", "interrupted", "blocked", "pawn", "crew", "auditor"]) {
                roles.append("Auditor")
            }
            if containsAny(normalized, ["safari", "chrome", "firefox", "arc browser", "spotify", "playlist", "browser", "computer", "screen", "click", "type into", "switch to", "close app", "quit app", "move window"]) {
                roles.append("Scout")
                roles.append("Auditor")
            }
        ...
        let externalAction = containsAny(normalized, [
            "calendar", "schedule", "reschedule", "gmail", "email", "create", "update", "change", "move my", "add to", "delete", "book", "purchase", "apply for",
            "safari", "chrome", "firefox", "arc browser", "spotify", "playlist", "browser", "computer", "screen", "click", "type into", "open app", "launch app", "switch to", "close app", "quit app", "move window",
        ])
    ...
    private static func taskClauses(_ command: String) -> [String] {
        let normalizedSeparators = command
    ...
    private static func acknowledgment(for command: String) -> (display: String, spoken: String) {
        if containsAny(command, ["safari", "chrome", "firefox", "arc browser", "spotify", "playlist", "browser", "computer", "screen", "click", "type into", "open app", "launch app", "switch to", "close app", "quit app", "move window"]) {
        ...
        if containsAny(command, ["research", "look up", "find", "compare", "recommend", "latest", "weather", "forecast", "temperature", "picture", "photo", "image", "diagram", "visualize"]) {
    ...
    private static func intent(for command: String) -> String {
        ...
        if containsAny(command, ["safari", "chrome", "firefox", "arc browser", "spotify", "playlist", "browser", "computer", "screen", "click", "type into", "open app", "launch app", "switch to", "close app", "quit app", "move window"]) { return "status" }
