...
public final class RookSpotifyClient {
  ...
  private func playForPurpose(_ purposes: [RookSpotifyPlaylistPurpose]) async throws -> RookResponse {
    ...
    if matches.count > 1, matches[1].score >= best.score - 7 {
      return playlistChoiceResponse(
        query: "a \(label) playlist",
        options: Array(matches.prefix(3).map(\.candidate.name))
      )
    }
    return try await play(query: best.candidate.name, preferredKind: .playlist, libraryOnly: true)
  }
  ...
  private func play(
    ...
    {
      return playlistChoiceResponse(query: ambiguousQuery, options: options)
    }
    let device = try await playbackDevice()
    let body: [String: Any]
  ...
  private func nowPlayingResponse() async throws -> RookResponse {
    ...
    let deviceName = playback.device?.name ?? "Spotify"
    return RookResponse(
      displayText: "**\(item.name)** by \(item.detail) is \(state.lowercased()) on **\(deviceName)**.",
      spokenText: "\(item.name) is \(state.lowercased()) on \(deviceName).",
      intent: "brief",
      requiresApproval: false,
      queueItemIDs: [],
      pawns: [],
