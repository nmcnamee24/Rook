...
public enum RookReflexCommandParser {
    ...
    private static func parseVolume(_ value: String) -> RookReflexIntent? {
        ...
        guard let groups = captures(
            ...
            in: value
        ), let percent = Double(groups[0]), (0...100).contains(percent) else { return nil }
    ...
    private static func parseConversion(_ value: String) -> RookConversion? {
        ...
        guard let groups = captures(
            ...
            in: candidate
              ...
              let from = unit(groups[1]), let to = unit(groups[2]), from.dimension == to.dimension else { return nil }
    ...
    private static func parseCalculation(_ value: String) -> RookCalculation? {
        ...
        guard let groups = captures(
            ...
            in: candidate
        ), let lhs = Double(groups[0]), let rhs = Double(groups[2]) else { return nil }
