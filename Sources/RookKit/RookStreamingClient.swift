...
public final class RookStreamingClient: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.noah.rook.streaming-client", qos: .userInitiated)
    ...
    private func handleMessage(_ message: JSON) {
        ...
        case "item/agentMessage/delta":
            guard let params = message["params"] as? JSON,
                  ...
                  matchesActiveTurn(params) else { return }
        ...
        case "turn/completed":
            guard let params = message["params"] as? JSON,
                  ...
                  completedTurnID == activeAnswer?.turnID else { return }
    ...
    private func startupFailed(_ message: JSON) {
        let detail = ((message["error"] as? JSON)?["message"] as? String)
    ...
    private func openLog() throws -> FileHandle {
        let url = config.appServerLogURL
        ...
        try handle.write(contentsOf: Data("\n[\(ISO8601DateFormatter().string(from: Date()))] START STREAMING FRONT\n".utf8))
        return handle
    }
    ...
    private func logTiming(_ event: String, request: AnswerRequest?) {
        ...
        let line = "[\(ISO8601DateFormatter().string(from: Date()))] \(event) \(request.id.uuidString.lowercased()) +\(String(format: "%.3f", elapsed))s\n"
    ...
    private func armAnswerWatchdog() {
        ...
        queue.asyncAfter(deadline: .now() + 12, execute: watchdog)
    }
