...
final class WakePhraseTests: XCTestCase {
    ...
    func testResponseSchemaDecodes() throws {
        let json = #"{"display_text":"Done","spoken_text":"Done.","intent":"status","requires_approval":false,"queue_item_ids":[],"pawns":[]}"#
    ...
    func testQuickResponseRoutesRoutineConversationWithoutPawns() throws {
        let json = #"{"display_text":"Hey — what’s up?","spoken_text":"Hey, what’s up?","route":"answer_now","intent":"answer","pawns":[]}"#
    ...
    func testQuickResponseShowsPlannedPawnsAsWorking() throws {
        let json = #"{"display_text":"I’m checking that now.","spoken_text":"I’m checking that now.","route":"deliberate","intent":"plan","pawns":[{"pawn":"Scout","task":"research the options"}]}"#
        let decoded = try JSONDecoder().decode(QuickRookResponse.self, from: Data(json.utf8))
    ...
    func testLegacyConfigGetsFastLayerAndPerPromptCrewDefaults() throws {
        let json = #"{"wake_phrase":"Rook","language":"en-US","on_device_only":true,"silence_seconds":0.7,"follow_up_window_seconds":3,"voice":"Samantha","voice_rate":225,"model":"gpt-5.6-terra","reasoning_effort":"high","max_pawns":3,"codex_path":"/tmp/codex","queue_script_path":"/tmp/queue.py","state_directory":"/tmp/rook"}"#
    ...
    func testAllPawnRolesRemainAvailableWhenLegacyConfigDisabledThem() {
        ...
        let quick = bridge.sanitized(QuickRookResponse(
        ...
        let deep = bridge.sanitized(RookResponse(
            displayText: "Done",
            spokenText: "Done.",
            intent: "status",
    ...
    func testInstantComputerParserRecognizesNarrowAppAndSpotifyControls() {
        ...
        XCTAssertEqual(RookComputerCommandParser.parse("Pause Spotify"), .spotify(.pause))
        XCTAssertEqual(RookComputerCommandParser.parse("Next track on Spotify"), .spotify(.next))
    }

    func testCompoundAndNamedPlaylistCommandsStayScreenAware() {
        ...
        let decision = LocalRookRouter.route("Play my Focus playlist on Spotify")
        XCTAssertEqual(decision.destination, .deliberate)
        XCTAssertEqual(decision.response.intent, "status")
        XCTAssertTrue(decision.response.pawns.contains { $0.pawn == "Auditor" })
    }
    ...
    func testBackgroundConnectorPolicyAllowsGuardedCalendarCreateUpdateAndGmailDraftWrites() {
        ...
        XCTAssertTrue(overrides.contains(where: { $0.contains("connector_947e0d954944416db111db556030eea6.destructive_enabled=true") }))
    ...
    func testLibraryCreatesConversationAndPawnTaskFolders() throws {
        ...
        let entry = try library.finishTurn(
            ...
            pawns: [PawnReport(pawn: "Steward", task: "checked calendar", status: "completed", id: "steward_1")]
        )
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: URL(fileURLWithPath: entry.conversationFolder).appendingPathComponent("summary.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: URL(fileURLWithPath: entry.taskFolder!).appendingPathComponent("pawns.json").path))
